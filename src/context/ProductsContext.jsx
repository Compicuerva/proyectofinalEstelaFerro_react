...
export const ProductsProvider = ({ children }) => {
  ...
  useEffect(() => {
    const cargarProductos = async () => {
      try {
        const respuesta = await fetch('https://68e0642893207c4b47945c02.mockapi.io/api/Productos');
  ...
  const editarProducto = async (productoActualizado) => {
    try {
      const respuesta = await fetch(`https://68e0642893207c4b47945c02.mockapi.io/api/Productos/${productoActualizado.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
